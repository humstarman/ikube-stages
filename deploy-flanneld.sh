...
FILE=/tmp/flannel-$FLANNEL_VER-linux-amd64.tar.gz
if [ ! -f $FILE ]; then
  while true; do
...
if [[ ! -x "$(command -v flanneld)" || ! -x "$(command -v mk-docker-opts.sh)" ]]; then
  while true; do
    ...
    ansible ${ANSIBLE_GROUP} -m copy -a "src=./flannel/bin/ dest=/usr/local/bin mode='a+x'"
    if [[ -x "$(command -v flanneld)" && -x "$(command -v mk-docker-opts.sh)" ]]; then
...
ansible ${ANSIBLE_GROUP} -m copy -a "src=./ssl/flanneld/ dest=/etc/flanneld/ssl"
